...
const sqlitePersistenceLesson = proofLesson({
  ...
  summary: "Query a pre-seeded SQLite sessions table to report total study minutes per topic.",
  ...
  objective: "Aggregate study sessions from a SQLite table to report total minutes per topic.",
  ...
  missionConnection: "This connects the Python path to the SQL path by aggregating the seeded session rows.",
  ...
  miniTitle: "Aggregate study minutes per topic",
...
sqlitePersistenceLesson.depth = {
  ...
  codeLabBridge: {
    story: "Now that parsed sessions live in a sessions table, we aggregate them with SQL to report total minutes per topic.",
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        prompt: "Your query `SELECT topic, SUM(minutes) FROM sessions` returns one combined total instead of one total per topic. What is missing?",
        choices: ["GROUP BY topic", "ORDER BY minutes", "A DISTINCT keyword"],
        correctChoiceIndex: 0,
        explanation: "Without GROUP BY topic, SUM(minutes) aggregates every session row into a single total. Adding GROUP BY topic returns one total row per topic.",
        conceptIds: ["py.sqlite"]
      },
      {
