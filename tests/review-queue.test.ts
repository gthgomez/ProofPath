...
describe("review queue", () => {
  ...
  it("hides stale persisted review items when the target is no longer completed", () => {
    const progress = {
      ...
      reviewItems: [createReviewItem("lesson", "lesson-python-functions", NOW)]
    };
  ...
  it("advances successful recall and creates repair work after again", () => {
    const progress = setRoleTarget(createInitialProgress(NOW), "path-software-foundations", true, NOW);
    const withLesson = setLessonCompletion(progress, "lesson-python-functions", true, NOW);
    const reviewed = recordReview(withLesson, "lesson", "lesson-python-functions", "good", "2026-05-05T17:00:00.000Z");
    const repeated = recordReview(reviewed, "lesson", "lesson-python-functions", "again", "2026-05-06T17:00:00.000Z");
  ...
  it("rewards successful current recall more than repeated again events", () => {
    const progress = setRoleTarget(createInitialProgress(NOW), "path-software-foundations", true, NOW);
    const withLesson = setLessonCompletion(progress, "lesson-python-functions", true, NOW);
    const again = recordReview(withLesson, "lesson", "lesson-python-functions", "again", "2026-05-05T17:00:00.000Z");
    const good = recordReview(withLesson, "lesson", "lesson-python-functions", "good", "2026-05-05T17:00:00.000Z");
    const staleGood = recordReview(withLesson, "lesson", "lesson-python-functions", "good", "2026-05-05T17:00:00.000Z");
    const roleContent = getContentForRole(contentPack, progress.profile.roleTargetId);
