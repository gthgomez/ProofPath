...
function createEvidenceProgress() {
  ...
  const withEvidence = addEvidenceItem(withDeliverables, {
    ...
    body: "Check output for the completed CLI Study Tracker mission.",
    ...
    reflection: "This evidence shows the tracker can be run and verified from a clean repo."
...
describe("weekly career reports", () => {
  it("generates evidence-backed weekly snapshots", () => {
    ...
    expect(report.summary).toContain("passing evidence");
  ...
  it("keeps same-week reports separate per role", () => {
    const junior = createEvidenceProgress();
  ...
  it("does not count legacy passing evidence without check output as passing evidence", () => {
    const roleProgress = setRoleTarget(createInitialProgress(NOW), "path-software-foundations", true, NOW);
      ...
      evidenceItems: [
        {
          ...
          body: "Old data said passing but had no check output.",
          linkedProjectMissionId: "mission-cli-study-tracker",
    ...
    expect(report.risks).toContain("No passing check-backed evidence is attached to this career path.");
  });
});
