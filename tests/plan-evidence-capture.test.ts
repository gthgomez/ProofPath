...
import { readFileSync, existsSync, writeFileSync, utimesSync, mkdirSync } from 'fs';
import os from 'os';
import path from 'path';

const SCRATCH = path.join(os.tmpdir(), 'proofpath-plan-evidence');
...
describe('plan evidence capture (tsc gating)', () => {
  beforeAll(() => {
    ...
    mkdirSync(path.dirname(TSC_LOG), { recursive: true });
